...
async function init() {
  ...
  const num = Math.max(...found);
  const boardI = found.indexOf(num);
  const picked = numbers.slice(0, num + 1);
  pSet = new Set(picked);
  last = picked[num];

  // console.log(num, boardI);
  // console.log(picked, last);

  const winning = boards[boardI].flat();
  // console.log(winning);
